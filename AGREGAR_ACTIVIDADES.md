...
Las nuevas actividades deben mantener una casita inferior derecha que apunte a `index.html` y guarde en `sessionStorage` la URL actual con las claves `ESL_LAST_ACTIVITY_URL`, `ESL_LAST_ACTIVITY_TITLE` y `ESL_HOME_FROM_ACTIVITY`. Así el menú puede mostrar el botón **Volver a la actividad**.
